...
echo   DWSIM EOS + Gelişmiş Kullanıcı Yönetimi
echo ============================================
echo.
...
if not exist "%SRC%kasp\core\libs\DWSIM.Thermodynamics.StandaloneLibrary.dll" (
    echo.  ⚠ WARNING: DWSIM DLL not found in kasp\core\libs\
    echo.    DWSIM EOS option will be disabled in the built EXE.
    echo.    Place DLL files in kasp\core\libs\ for DWSIM support.
    echo.
)

echo [2/4] Copying updated source files to build directory...
...
echo.
echo [4/4] Copying EXE back to project...
...
echo   %SRC%dist\
echo.
echo   SHA-256 (compute manually):
echo   certutil -hashfile "%SRC%dist\%RELEASE_EXE%" SHA256
echo ============================================
pause
