...
def test_create_user(user_mgr):
    ...
    user, err = user_mgr.create_user("engineer1", "Eng12345", role="engineer", full_name="Test Eng")
    assert user is not None
    assert err is None
...
def test_create_duplicate_user(user_mgr):
    ...
    user_mgr.create_user("dup_user", "Pw123456")
...
def test_create_user_empty_username(user_mgr):
    ...
    user, err = user_mgr.create_user("", "Pw123456")
    assert user is None
    assert err is not None
...
def test_update_user(user_mgr):
    ...
    user, _ = user_mgr.create_user("editme", "Pw123456", role="user")
...
def test_delete_user(user_mgr):
    ...
    user, _ = user_mgr.create_user("delete_me", "Pw123456")
    assert user_mgr.delete_user(user.id)
    assert user_mgr.authenticate("delete_me", "Pw123456") is None


def test_toggle_user_active(user_mgr):
    """Kullanıcı aktif/pasif toggle edilebilmeli."""
    user, _ = user_mgr.create_user("toggle_me", "Pw123456")
...
def test_change_password_valid(user_mgr):
    ...
    user, _ = user_mgr.create_user("pw_user", "Old12345")
    ok, err = user_mgr.change_password(user.id, "Old12345", "New56789")
    assert ok
    assert err is None
    assert user_mgr.authenticate("pw_user", "New56789") is not None
    assert user_mgr.authenticate("pw_user", "Old12345") is None
...
def test_authenticate_updates_last_login(user_mgr):
    ...
    user_mgr.create_user("login_tester", "Pw123456")
    user = user_mgr.authenticate("login_tester", "Pw123456")
...
def test_admin_can_clear_must_change_password(user_mgr):
    ...
    user, _ = user_mgr.create_user("flagged", "Pw123456")
    user_mgr.admin_reset_password(user.id, "Newpw9999")
    users = user_mgr.list_users()
