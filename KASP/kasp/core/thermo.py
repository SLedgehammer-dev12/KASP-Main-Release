...
class ThermoEngine:
    ...
    def _calculate_design_energy_context(self, inputs, context, stage_loop):
        ...
        mass_flow = context["mass_flow_per_unit"]
        inlet_Q_m3s = mass_flow / state_inlet.density if state_inlet.density > 0 else 0.0
        dimension = CompressorAerodynamics.calculate_dimensionless(
            total_poly_head_kj_kg * 1000.0,
            inlet_Q_m3s,
            state_inlet,
        )
        ...
        try:
            ...
            try:
                ...
                results = build_design_results_payload(
                    ...
                    dimensionless=energy.get("dimensionless", {}),
                    num_stages=context["num_stages"],
                    staged_results=staged_results,
    ...
    def _calculate_with_consistency(self, inputs):
        ...
        for iteration in range(max_iter):
            ...
            if len(history) >= 2:
                prev_residual = history[-2]["residual"]
                if residual < prev_residual * 0.5:
                    relaxation = min(0.7, relaxation * 1.25)
                elif residual > prev_residual * 1.5:
                    relaxation = max(0.15, relaxation * 0.6)
