...
class ThermodynamicSolver:
    ...
    def __init__(self, max_cache_size=2000):
        ...
        # Cache for CoolProp phase envelopes (compute once per composition)
    ...
    def _convert_coolprop_string_to_dict(self, mixture_string: str) -> dict:
        """CoolProp karışım string'ini Thermo/AGA8 dict formatına çevir.
        
        'Methane[0.98]&Ethane[0.015]' → {'ids':['methane','ethane'], 'mol_fractions':[0.98,0.015]}
        """
        ...
        for match in pattern.finditer(mixture_string):
            name = match.group(1)
            frac = float(match.group(2))
            ids.append(GasMixtureBuilder.THERMO_ID_MAP.get(name.upper(), name.lower()))
            fractions.append(frac)

        if not ids:
            raise ValueError(f"CoolProp karışım string'i ayrıştırılamadı: {mixture_string[:50]}")

        return {"ids": ids, "mol_fractions": fractions}

    def _get_reducing_temperature(self, mixture_string: str) -> float | None:
        """Karışım için psödo-kritik sıcaklığı (T_reducing) önbellekten al."""
        if not COOLPROP_LOADED:
            return None

        gas_hash = self._build_gas_hash(mixture_string)
        with self._envelope_lock:
            if gas_hash in self._envelope_cache:
                return self._envelope_cache[gas_hash]

        try:
            t_reducing = CP.PropsSI("T_reducing", mixture_string)
            with self._envelope_lock:
                self._envelope_cache[gas_hash] = t_reducing
            logger.debug(
                "Mixture reducing temperature: %.1f K (ratio at 300K: %.2f)",
                t_reducing,
                300.0 / t_reducing,
            )
            return t_reducing
        except Exception as err:
            logger.warning("T_reducing lookup failed for %s: %s", mixture_string[:40], err)
            return None

    @staticmethod
    def _determine_phase_coolprop(T_k: float, P_pa: float, Z: float, D: float,
                                   t_reducing: float | None) -> str:
        """CoolProp karışımları için faz tespiti.

        T_reducing (psödo-kritik sıcaklık) tabanlı güvenilir yaklaşım:
        - Kompresör uygulamalarında T_çalışma ≫ T_reducing olduğundan
          çalışma noktası her zaman gaz/süperkritik bölgededir.
        """
        if t_reducing is not None and T_k > t_reducing * 1.15:
            return "gas"

        if D < 150.0 and Z > 0.3:
            return "gas"
        elif D > 500.0 and Z < 0.2:
            return "liquid"
        elif D > 150.0 and Z < 0.25:
            return "dense_phase"
        else:
            return "gas"
    ...
    def get_properties(self, P_pa: float, T_k: float, gas_obj, eos_method: str) -> ThermodynamicState:
        ...
        try:
            ...
            logger.warning(f"⚠️ {eos_method.upper()} EOS hatası: {e}. Fallback başlatılıyor.")

            # CoolProp string gas_obj'yi dict formatına çevir (PR/SRK/AGA8 string kabul etmez)
            if isinstance(gas_obj, str):
                try:
                    dict_gas = self._convert_coolprop_string_to_dict(gas_obj)
                except Exception:
                    dict_gas = gas_obj  # çevrilemezse olduğu gibi dene
            else:
                dict_gas = gas_obj

            if eos_method == 'coolprop':
                # CoolProp → AGA8 (GERG-2008) → PR → İdeal Gaz
                try:
                    state = self._solve_aga8(P_pa, T_k, dict_gas)
                    state.raw_props['fallback'] = True
                    state.raw_props['fallback_type'] = 'aga8_fallback'
                    logger.info("  ↳ CoolProp → AGA8 fallback başarılı.")
                except Exception:
                    try:
                        state = self._solve_thermo_eos(P_pa, T_k, dict_gas, 'pr')
                        state.raw_props['fallback'] = True
                        state.raw_props['fallback_type'] = 'pr_fallback'
                        logger.info("  ↳ CoolProp → AGA8 başarısız → PR fallback başarılı.")
                    except Exception as fallback_err:
                        logger.warning(f"  ↳ PR da başarısız: {fallback_err}. İdeal Gaz'a geçiliyor.")
                ...
                try:
                    ...
                    state.raw_props['fallback'] = True
                    state.raw_props['fallback_type'] = 'pr_fallback'
                    logger.info("  ↳ AGA8 → PR fallback başarılı.")
                except Exception as fallback_err:
                    logger.warning(f"  ↳ PR da başarısız: {fallback_err}. İdeal Gaz'a geçiliyor.")
                    state = self._solve_fallback(P_pa, T_k, gas_obj, eos_method)
    ...
    def _solve_coolprop(self, P_pa: float, T_k: float, mixture_string: str) -> ThermodynamicState:
        ...
        # Faz tespiti: T_reducing (psödo-kritik sıcaklık) tabanlı
        # Kompresör proses şartlarında T_çalışma ≫ T_reducing → gaz
        t_reducing = self._get_reducing_temperature(mixture_string)
        phase_str = self._determine_phase_coolprop(T_k, P_pa, Z, D, t_reducing)
    ...
    def _solve_aga8(self, P_pa: float, T_k: float, gas_data: dict) -> ThermodynamicState:
        ...
        # Faz denetimi: Z + yoğunluk tabanlı güvenilir sezgisel
        phase_str = 'gas'
        if Z < 0.15 and density > 400.0:
            phase_str = 'liquid'
        elif Z < 0.25 and density > 200.0:
            phase_str = 'dense_phase'
    ...
    def _solve_fallback(self, P_pa: float, T_k: float, gas_obj, eos: str) -> ThermodynamicState:
        """Kütüphane başarısız olduğunda ideal gaz yaklaşımı. Bileşen kompozisyonundan kütle-ağırlıklı Cp hesaplar."""
        from kasp.core.constants import IDEAL_GAS_CP_300K, normalize_component
        ...
        # Bileşen-bazlı mol-ağırlıklı Cp hesabı (J/mol·K)
        if isinstance(gas_obj, dict):
            ids = gas_obj.get("ids", gas_obj.get("IDs", []))
            zs = gas_obj.get("mol_fractions", gas_obj.get("zs", []))
            if ids and zs and len(ids) == len(zs):
                cp_molar = sum(
                    float(zs[i]) * IDEAL_GAS_CP_300K.get(
                        normalize_component(str(ids[i])), 35.0
                    )
                    for i in range(len(zs))
                )
                Cp_ideal = cp_molar / M_kg_mol  # J/mol·K → J/kg·K
            else:
                Cp_ideal = 1005.0  # hava varsayılan
        elif isinstance(gas_obj, str):
            # CoolProp karışım string'i — metan-ağırlıklı doğal gaz varsay
            Cp_ideal = 2200.0
        else:
            Cp_ideal = 1005.0

        # Sıcaklık bağımlılığı (T^0.08 ≈ %3-5 artış @ 500K)
        t_ref = max(50.0, T_k)
        Cp_ideal *= (t_ref / 300.0) ** 0.08

        Cv_ideal = Cp_ideal - R_specific
        k_ideal = Cp_ideal / Cv_ideal if Cv_ideal > 0 else 1.4

        # Negatif/çok düşük sıcaklıklar için fiziksel alt sınır (1 K)
        t_safe = max(1.0, T_k)
        Z_ideal = max(0.5, min(1.5, 1.0 - 0.1 * (P_pa / (STD_PRESS_PA * 10))))
        rho_ideal = P_pa / (R_specific * t_safe * Z_ideal) if R_specific > 0 else 1.0

        H_ideal = Cp_ideal * (t_safe - 273.15)
        S_ideal = Cp_ideal * math.log(t_safe / 273.15) if t_safe > 0 else 0
