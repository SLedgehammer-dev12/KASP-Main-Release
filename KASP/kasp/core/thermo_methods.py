...
class ThermoMethodSuite:
    ...
    def method_direct_hs(self, p_in, t_in, p_out, poly_eff, gas_obj, eos):
        ...
        for outer in range(outer_iterations):
            n_poly = (k_avg - 1) / (k_avg * poly_eff)
            if abs(n_poly) < 1e-10:
                n_poly = 1e-6

            poly_head_kj = self._calculate_polytropic_head(
                z1, r_specific, t_in, pressure_ratio, n_poly
            )
            delta_h_actual_est = (poly_head_kj * 1000.0) / max(poly_eff, 0.01)

            eta_isen = delta_h_isen / max(delta_h_actual_est, 1.0)
