...
class CompressorAerodynamics:
    ...
    def calculate_isentropic_outlet_temp(state_in: ThermodynamicState, p_out: float,
        ...
        Aşama zinciri:
        1. CoolProp direkt P-S flash (en hızlı, en doğru)
        2. Üçlü çözücü zinciri: FD-NR → AJ-NR → Brent (CoolProp başarısızsa
           veya PR/SRK/AGA8 kullanılıyorsa)
        """
        # 1. CoolProp direkt entropy-pressure flash
        if eos == 'coolprop':
            try:
                import CoolProp.CoolProp as CP
                t_isen = CP.PropsSI('T', 'P', p_out, 'Smass', state_in.S, gas_obj)
                if t_isen > 0 and t_isen >= state_in.T * 0.5:
                    return t_isen
            except Exception as e:
                logger.debug(f"Direct Isentropic Flash failed ({e}). Proceeding to three-solver fallback.")
        else:
            logger.debug(f"EOS={eos}: using three-solver isentropic fallback chain.")

        # 2. Üçlü çözücü zinciri (FD-NR → AJ-NR → Brent) — en güvenilir sonuç
    ...
    @staticmethod
    def estimate_impeller_geometry(head_pol_jkg, Q_m3s):
        u_est = math.sqrt(2 * head_pol_jkg / 0.50)
        D_est = math.sqrt(4 * Q_m3s / (math.pi * u_est * 0.06))
        N_est = u_est * 60 / (math.pi * D_est)
        b_est = D_est * 0.05
        return {
            "u_tip_ms": u_est,
            "D_m": D_est,
            "b_m": b_est,
            "speed_rpm": N_est,
            "omega_rad_s": u_est * 2 / D_est,
        }

    @staticmethod
    def calculate_dimensionless(head_pol_jkg, Q_m3s, state_in, speed_rpm=None, D_m=None, b_m=None):
        if D_m is None or speed_rpm is None or b_m is None:
            geom = CompressorAerodynamics.estimate_impeller_geometry(head_pol_jkg, Q_m3s)
            D_m = D_m or geom["D_m"]
            b_m = b_m or geom["b_m"]
            speed_rpm = speed_rpm or geom["speed_rpm"]
            estimated = True
        else:
            estimated = False
        ...
        psi = head_pol_jkg / (u * u / 2) if u > 0 else 0.0
        phi = Q_m3s * 4 / (math.pi * D_m * D_m * u) if u > 0 and D_m > 0 else 0.0
        phi_3 = Q_m3s / (math.pi * D_m * b_m * u) if u > 0 and D_m > 0 and b_m > 0 else 0.0

        mu = state_in.raw_props.get("mu", 1.1e-5)
        Re = u * b_m * state_in.density / mu if mu > 0 else 0.0
        a_sound = state_in.raw_props.get("speed_of_sound")
        if a_sound is None:
            a_sound = math.sqrt(max(state_in.k * state_in.P / state_in.density, 0.0))
        Ma = u / a_sound if a_sound > 0 else 0.0
