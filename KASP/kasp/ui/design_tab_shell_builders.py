...
def get_design_left_scroll_width_range():
    # V4.7: Responsive width based on DPI scale factor
    try:
        from kasp.ui.responsive import scaled, current_breakpoint
        bp = current_breakpoint()
        if bp.category in ("XS", "SM"):
            return (scaled(260), scaled(360))
        return (scaled(360), scaled(520))
    except Exception:
        return (360, 520)
