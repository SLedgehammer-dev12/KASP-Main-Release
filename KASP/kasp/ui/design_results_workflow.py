...
class DesignResultsPresenter:
    ...
    def populate_detailed_tables(self, results):
        ...
        dimless = results.get("dimensionless", {})
        if dimless and dimless.get("psi", 0) > 0:
            self.window.thermo_table.setRowCount(len(thermo_props) + 8)
            dim_rows = [
                ("Head Katsayısı (ψ)", f"{dimless.get('psi', 0):.4f}", "-", "Tahmini" if dimless.get("estimated") else ""),
                ("Debi Katsayısı (φ)", f"{dimless.get('phi', 0):.4f}", "-", ""),
                ("Çıkış Katsayısı (φ₃)", f"{dimless.get('phi_3', 0):.4f}", "-", ""),
                ("Reynolds (Re)", f"{dimless.get('Re', 0):.2e}", "-", ""),
                ("Mach (Ma)", f"{dimless.get('Ma', 0):.4f}", "-", ""),
                ("Tahmini Çark Çapı", f"{dimless.get('D_m', 0)*1000:.0f}", "mm", ""),
                ("Tahmini Çark Hızı", f"{dimless.get('speed_rpm', 0):.0f}", "RPM", ""),
                ("Çark Uç Hızı", f"{dimless.get('u_tip_ms', 0):.0f}", "m/s", ""),
            ]
            start_row = len(thermo_props)
            for i, (name, val, unit, note) in enumerate(dim_rows):
                row = start_row + i
                name_text = name if not note else f"{name} {note}"
                self.window.thermo_table.setItem(row, 0, QTableWidgetItem(name_text))
                self.window.thermo_table.setItem(row, 1, QTableWidgetItem(val))
                self.window.thermo_table.setItem(row, 2, QTableWidgetItem("-"))
                self.window.thermo_table.setItem(row, 3, QTableWidgetItem(unit))
                self.window.thermo_table.setItem(row, 4, QTableWidgetItem("-"))
