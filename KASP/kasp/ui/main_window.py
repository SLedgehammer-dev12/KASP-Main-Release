...
class KaspMainWindow(QMainWindow):
    def __init__(self):
        ...
        # V4.7: Responsive — DPI-aware size, centering, geometry persistence
        try:
            from kasp.ui.responsive import (
                compute_initial_window_size,
                compute_centered_position,
                compute_minimum_window_size,
                get_screen_geometry,
                responsive_base_stylesheet,
            )
            min_w, min_h = compute_minimum_window_size()
            self.setMinimumSize(min_w, min_h)

            cfg = get_config_manager()
            saved_w = cfg.get("ui.window_width", 0)
            saved_h = cfg.get("ui.window_height", 0)
            _w, _h = compute_initial_window_size(1700, 950)
            # Only use saved geometry if it fits on the current screen
            if saved_w >= 900 and saved_h >= 550:
                sw, sh = get_screen_geometry()
                if saved_w <= sw * 0.95 and saved_h <= sh * 0.93:
                    _w, _h = saved_w, saved_h
            _x, _y = compute_centered_position(_w, _h)
            self.setGeometry(_x, _y, _w, _h)

            # Apply DPI-aware stylesheet (best-effort)
            try:
                self.setStyleSheet(responsive_base_stylesheet())
            except Exception:
                pass
        except Exception:
            # Bare-minimum fallback: center a reasonable default
            self.setMinimumSize(900, 550)
            try:
                from kasp.ui.responsive import compute_centered_position
                self.resize(1700, 950)
                _x, _y = compute_centered_position(self.width(), self.height())
                self.move(_x, _y)
            except Exception:
    ...
    def closeEvent(self, event):
        # V4.7: Persist window geometry for next session (skip in tests)
        if not os.environ.get("KASP_TEST_MODE"):
            try:
                from kasp.config_manager import get_config_manager
                cfg = get_config_manager()
                cfg.set("ui.window_width", self.width())
                cfg.set("ui.window_height", self.height())
            except Exception:
                pass
    ...
    def _apply_saved_settings(self):
        ...
        from kasp.i18n import tr, apply_window_language

        config = get_config_manager()
        theme = config.get("app.theme", "light")
        lang = config.get("app.language", "tr")
        ...
        self.setWindowTitle(tr(f"KASP v{APP_VERSION} - Termodinamik Analiz"))
    ...
    def _on_switch_theme(self, theme_name):
        ...
        self._update_theme_menu_checks(theme_name)

    def _on_switch_language(self, lang):
        from kasp.config_manager import get_config_manager
        from kasp.i18n import set_language, tr, apply_window_language

        set_language(lang)
        apply_window_language(self)

        self.setWindowTitle(tr(f"KASP v{APP_VERSION} - Termodinamik Analiz"))
        self._update_language_menu_checks(lang)

    def _update_theme_menu_checks(self, theme_name):
        if hasattr(self, "_theme_actions"):
            for value, action in self._theme_actions.items():
                action.setChecked(value == theme_name)

    def _update_language_menu_checks(self, lang):
        if hasattr(self, "_language_actions"):
            for value, action in self._language_actions.items():
                action.setChecked(value == lang)
