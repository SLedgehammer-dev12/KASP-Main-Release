...
_REVERSE_EXACT = {v: k for k, v in _EXACT_TRANSLATIONS.items()}
_REVERSE_SUBSTRING = {v: k for k, v in _SUBSTRING_TRANSLATIONS.items()}
...
def tr(text: str) -> str:
    ...
    if is_english():
        ...
        translated = _REVERSE_EXACT.get(text, text)
        for source, target in _REVERSE_SUBSTRING.items():
            translated = translated.replace(source, target)
        return translated
