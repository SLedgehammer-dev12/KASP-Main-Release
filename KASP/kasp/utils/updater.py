...
def pick_default_asset(release: ReleaseInfo) -> ReleaseAsset | None:
    if not release.assets:
        return None
    # Platform-aware: prefer .dmg on macOS, .exe on Windows, .app.zip fallback
    if _is_macos():
        dmg = next((a for a in release.assets if a.name.lower().endswith(".dmg")), None)
        if dmg:
            return dmg
        app_zip = next((a for a in release.assets if ".app" in a.name.lower() and a.name.lower().endswith(".zip")), None)
        if app_zip:
            return app_zip
    exe = next((a for a in release.assets if a.name.lower().endswith(".exe")), None)
    return exe or release.assets[0]
